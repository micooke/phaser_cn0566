...
class phaser_SoapySDR:
    ...
    def buffer_read(self):
        _read_status = self.sdr.readStream(self.rxStream, [self.buffer], self.rx_buffer_size)
    ...
    def plot(fc_Hz: float = Phaser_LO_HIGH, DeviceString: str = "hackrf"):
        ...
        sdr = phaser_SoapySDR(fc_Hz=fc_Hz, DeviceString = DeviceString)
        if sdr.sdr is None:
            print(f"[ERROR] {sdr.__name__} not found")
            return
...
if __name__ == '__main__':
    ...
    num_reads=5
    DeviceString='hackrf'#'bladerf'#'plutosdr'
    phaser_SoapySDR.plot(fc_Hz = 1.6e9, DeviceString=DeviceString)
    phaser_SoapySDR.test(num_reads=num_reads, DeviceString=DeviceString)
    phaser_SoapySDR.record(num_reads=num_reads, DeviceString=DeviceString)
